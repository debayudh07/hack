...
export default function Homepage() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-black text-sky-200">
      <header className={`fixed top-0 left-0 right-0 z-50 px-4 lg:px-6 h-14 flex items-center justify-between transition-all duration-300 ${isScrolled ? 'bg-gray-900 shadow-md' : 'bg-transparent'}`}>
        <Link className="flex items-center justify-center" href="#">
          <BrainCircuit className="h-6 w-6 text-sky-300" />
          <span className="ml-2 text-lg font-bold text-sky-200">AIax</span>
        </Link>
        <nav className="hidden md:flex gap-4 sm:gap-6">
          <Link className="text-sm font-medium hover:text-sky-300 transition-colors" href="/portfolio">
            Portfolio
          </Link>
        ...
        {isMenuOpen && (
          ...
          >
            <div className="flex flex-col h-full p-4">
              <Button variant="ghost" size="icon" className="self-end text-sky-200 hover:text-sky-300" onClick={toggleMenu}>
                <X />
                <span className="sr-only">Close menu</span>
              </Button>
              <nav className="flex flex-col items-center justify-center flex-1 gap-4">
                <Link className="text-lg font-medium hover:text-sky-300 transition-colors" href="/portfolio" onClick={toggleMenu}>
                  Portfolio
                </Link>
              ...
              {[
                {
                  ...
                  description: "Automatically categorize and track your expenses with AI precision."
                },
                {
                  icon: <PieChart className="h-12 w-12 mb-4 text-sky-400" />,
                  title: "Personalized Insights",
                  description: "Get tailored financial advice based on your spending patterns."
                },
                {
                  icon: <TrendingUp className="h-12 w-12 mb-4 text-sky-400" />,
                  title: "Automated Savings",
                  description: "Let AI optimize your savings with smart allocation strategies."
                }
              ].map((feature, index) => (
                <motion.div
                  key={index}
                ...
                >
                  {feature.icon}
                  <h3 className="text-xl font-bold mb-2 text-sky-200">{feature.title}</h3>
            ...
            <div className="flex flex-col items-center justify-center space-y-4 text-center">
              ...
              <div className="w-full max-w-sm space-y-2">
                <form className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2">
                  <Input className="max-w-lg flex-1 bg-gray-800 border-gray-700 text-sky-200 placeholder-sky-400" placeholder="Enter your email" type="email" />
                  <Button type="submit" className="bg-sky-500 text-black hover:bg-sky-400 transition-colors">Sign Up</Button>
                </form>
      ...
      <footer className="flex flex-col gap-2 sm:flex-row py-6 w-full shrink-0 items-center px-4 md:px-6 border-t border-gray-800">
        <p className="text-xs text-sky-400">© 2023 AI Finance. All rights reserved.</p>
        <nav className="sm:ml-auto flex gap-4 sm:gap-6">
          <Link className="text-xs hover:text-sky-300 transition-colors" href="#">
            Terms of Service
          </Link>
          <Link className="text-xs hover:text-sky-300 transition-colors" href="#">
            Privacy
